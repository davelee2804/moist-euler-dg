...
def pot_temp(rho, eta, q_v, q_l, q_i):
    eta_d = eta_k_to_eta_d(rho, eta, q_v, q_l, q_i) 
    ...
    # Eldred et al., QJRMS (2022) eqn. 70
    theta = T_0 * np.exp((eta_d - eta_0)*c_p_inv) \
                * np.power(p_0d * R_inv, R * c_p_inv) \
                * np.power(q_d * R_d / p_0d, q_d * R_d * c_p_inv) \
                * np.power(q_v * R_v / p_0v, q_v * R_v * c_p_inv)

    return theta

class _NonEqEuler2D(Euler2D):
    nvars = 7
